                    ...
                    <div class="row" v-if="responseAvailable">
                        ...
                        <div class="col-9">
                             <table class="table">
                                 ...
                                 <tbody>
                                     <tr v-for="(pagecontent,index) in pagecontents" v-bind:key="index">
                                         <td>{{index+1}}</td>
                                         <td>{{pagecontent.contentType}}</td>
                                         <td>{{pagecontent.pageId.pageName}}</td>
                                         <td>{{pagecontent.contentHeading}}</td>
                                         <td>
                                            <router-link :to="{name:'Edit Page Content',params:{id:pagecontent._id}}" >Edit</router-link> &nbsp;
                                            <a href="" v-on:click="DeletePageContent(pagecontent._id,index)">Delete</a>
                                         </td>
                                     </tr>
                                 </tbody>
                             </table>
...
<script>
    ...
    export default {
       ...
       methods:{
        ...
        mounted(){
            axios.get("http://52.66.202.99:8080/api/admin/pagecontent/list")
                .then(response=>{
